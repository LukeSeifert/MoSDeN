...
There is also the presence of $^{85}$As, which did not appear in the top six \glspl{DNP} when using the IAEA $\beta$-delayed neutron emission database with JENDL-5 cumulative fission yields.
...
These differences are significant in the context of reactor transients which evolve over fractions of seconds and hundreds of pcm.
...
These results show that several \glspl{DNP} have a stronger correlation than $^{87}$Br, which is surprising considering its long half-life causes group one \gls{DNP} group parameters to be very sensitive to it.
...
Figure \ref{fig:b71-bar-uiv} gives a more detailed view of which \gls{DNP} values specifically lead to the large $U_i$ values.
...
\subfloat[The $PCC_i$, or total \gls{PCC}, calculated for each \gls{DNP}.]{
  \includegraphics[width=0.65\linewidth]{images/b71/chart_PCC.png}
}
\\
\subfloat[The $U_i$, or total uncertainty-weighted \gls{PCC}, for each \gls{DNP}.]{
  \includegraphics[width=0.65\linewidth]{images/b71/chart_PCC_uncertainty.png}
}
