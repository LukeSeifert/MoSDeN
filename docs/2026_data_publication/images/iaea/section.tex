...
Figure \ref{fig:count-rate-iaea-jendl}(a) shows the histogram for the sampled group one half-lives $\tau_1$ as well as how the delayed neutron count rate compares to those using \gls{DNP} group parameters from the literature \cite{keepin1957delayed, brady1989delayed, synetos1983delayed}.
...
Figure \ref{fig:count-rate-iaea-jendl}(b) shows the Keepin normalized delayed neutron count rate from the Keepin et al. six \gls{DNP} group parameter fit \cite{keepin1957delayed}.
The figure compares the delayed neutron count rate calculated in this work both for every count rate from the sampled data and from the group fit using the calculated \gls{DNP} group parameters.
...
This is why the Keepin 6-group data is unity at all times, as it is always self-equivalent.
These results show that the Keepin et al. \gls{DNP} group parameters agree well with group parameters calculated using the \gls{IAEA} $\beta$-delayed neutron emission database and JENDL-5 cumulative fission yields, with differences smaller than those from Brady and England \cite{brady1989delayed} and from Synetos and Williams \cite{synetos1983delayed}.
...
\subfloat[The delayed neutron count rate as a function of time relative to the Keepin et al. \gls{DNP} group parameter delayed neutron count rate (such that the Keepin 6-Group Fit is unity at all points in time). Comparison is made between the six \gls{DNP} group parameters from this work, Brady and England, and Synetos and Williams \cite{keepin1957delayed, brady1989delayed, synetos1983delayed}.]{
  \includegraphics[width=0.45\linewidth]{images/iaea/Keepin_counts.png}
}
\caption{
%These plots show how the 5000 samples lead to a distribution of \gls{DNP} group parameters and delayed neutron count rates from . 
These plots show the distribution of the 5000 randomly sampled results using JENDL-5 cumulative fission yields and IAEA $\beta$-delayed neutron emission database half-lives and emission probabilities.
The \gls{DNP} group one half-life $\tau_1$ shows a normal distribution about a mean, while the Keepin et al. normalized count rate shows this work offers good agreement with the literature.}
\label{fig:count-rate-iaea-jendl}
\end{figure}

Figure \ref{fig:sens-iaea-jendl} shows two examples of how the \glspl{PCC} are calculated using the 5000 samples.
...
\begin{table}[]
    ...
    \begin{tabular}{lll}
        ...
        $\Delta \nu_d$ & $0.03 \pm 122.82$ & $[pcm]$\\
        \hline
        $\bar{\tau} (I)$ & $9.1 \pm 0.5$ & $[s]$\\
        $\bar{\tau} (K)$ & $9.0 \pm 0.4$ & $[s]$\\
        $\Delta \bar{\tau}$ & $0.0 \pm 0.6$ & $[s]$\\ %0.005096 +/- 0.649
        \hline
    \end{tabular}
    \label{tab:iaea-jendl5-yld-hl}
\end{table}

Table \ref{tab:iaea-jendl5-PCCi} shows the ten largest $PCC_i$ terms, demonstrating which \glspl{DNP} have the greatest impact on the \gls{DNP} group parameters $\nu_{d,k}$ and $\tau_k$.
...
Generally, \glspl{DNP} will have a large $PCC_i$ for one of two reasons: they affect several parameters moderately, such as $^{96}$Rb which has an impact on group six as well as a smaller impact on groups five, four, three, and two as well; or they have a half-life that causes them to have a large impact on one specific group, such as $^{87}$Br, which primarily affects group one.
...
Interestingly, it appears that cobalt has several isotopes that have large $U_i$ values, indicating that cobalt in this nuclear dataset has relative uncertainties that lead to larger differences in the \gls{DNP} group parameters than other \glspl{DNP}.
...
Figure \ref{fig:iaea-jendl-bar-uiv} gives a more detailed view of which \gls{DNP} values specifically lead to the large $U_i$ values.
...
These plots are not limited to only the top ten largest values, but instead show how all of the values compare with each other.
...
\subfloat[The $PCC_i$, or total \gls{PCC}, calculated for each \gls{DNP}.]{
  \includegraphics[width=0.65\linewidth]{images/iaea/chart_PCC.png}
}
%\hspace{0.05\linewidth}
\\
\subfloat[The $U_i$, or total uncertainty-weighted \gls{PCC}, for each \gls{DNP}.]{
  \includegraphics[width=0.65\linewidth]{images/iaea/chart_PCC_uncertainty.png}
}
