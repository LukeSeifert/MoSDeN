...
The dataset used has a large effect on the total delayed neutron yield and average half-life values.
...
The results reflect those shown in Figure \ref{fig:hybrid-pure-compare}: the ENDF datasets are within uncertainty bounds for the different versions, while the JEFF-3.1.1 and JENDL-5 datasets disagree with ENDF and agree with each other.
...
It appears in Figure \ref{fig:hybrid-nu-heatmap} that the cumulative fission yield differences drive the differences in the total delayed neutron yields, with smaller differences from the emission probabilities.
...
\hspace{0.05\linewidth}
\subfloat[The JEFF-3.1.1 \gls{DNP} $\nu_{d_i}$ minus the JENDL-5 \gls{DNP} $\nu_{d,i}$ for every \gls{DNP} present in either dataset, where $\nu_{d,i}$ is set to zero if it is not present in the dataset.]{
